...
fn test_is_symmetric() {
    ...
    for (i, (input, output)) in cases.into_iter().enumerate() {
        assert_eq!(p2::is_symmetric(TreeNode::from_slice(input.as_slice())), output, "case {} failed", i);
    }
}
...
fn test_max_depth() {
    ...
    for (i, (input, output)) in cases.into_iter().enumerate() {
        assert_eq!(p2::max_depth(TreeNode::from_slice(input.as_slice())), output, "case {} failed", i);
    }
}
...
fn test_is_balanced() {
    ...
    for (i, (input, output)) in cases.into_iter().enumerate() {
        assert_eq!(p2::is_balanced(TreeNode::from_slice(input.as_slice())), output, "case {} failed", i);
    }
}
...
fn test_min_depth() {
    let cases = vec![
        ...
        (vec![2, i32::MAX, 3, i32::MAX, 4, i32::MAX, 5, i32::MAX, 6], 5),
        (vec![], 0),
        (vec![1], 1),
    ];

    for (i, (input, output)) in cases.into_iter().enumerate() {
        assert_eq!(p2::min_depth(TreeNode::from_slice(input.as_slice())), output, "case {} failed", i);
    }
}
...
fn test_has_path_sum() {
    let cases = vec![
        (vec![5,4,8,11,i32::MAX,13,4,7,2,i32::MAX,i32::MAX,i32::MAX,1],22,true),
        (vec![1,2,3],5,false),
        (vec![], 0, false),
        (vec![1],1,true),
    ];
    for (i, (input, target, output)) in cases.into_iter().enumerate() {
        assert_eq!(p2::has_path_sum(TreeNode::from_slice(input.as_slice()), target), output, "case {} failed", i);
    }
}
...
fn test_is_palindrome() {
    ...
    for (i, (input, output)) in cases.into_iter().enumerate() {
        assert_eq!(p2::is_palindrome(input.to_string()), output, "case {} failed", i);
    }
}
...
fn test_preorder_traversal() {
    ...
    for (i, (input, output)) in cases.into_iter().enumerate() {
        assert_eq!(p2::preorder_traversal(TreeNode::from_slice(input.as_slice())), output, "case {} failed", i);
    }
}
...
fn test_postorder_traversal() {
    ...
    for (i, (input, output)) in cases.into_iter().enumerate() {
        assert_eq!(p2::postorder_traversal(TreeNode::from_slice(input.as_slice())), output, "case {} failed", i);
    }
}
