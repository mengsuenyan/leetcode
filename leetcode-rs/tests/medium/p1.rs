...
fn test_add_two_numbers() {
    ...
    for (i, (l1, l2, output)) in cases.into_iter().enumerate() {
        assert_eq!(p1::add_two_numbers(ListNode::from_slice(l1.as_slice()), ListNode::from_slice(l2.as_slice())), ListNode::from_slice(output.as_slice()), "case {} failed", i);
    }
...
fn test_length_of_longest_substring() {
    ...
    for (i, (input, output)) in cases.into_iter().enumerate() {
        assert_eq!(p1::length_of_longest_substring(input.to_string()), output, "case {} failed", i);
    }
}
...
fn test_longest_palindrome() {
    ...
    for (i, (input, output)) in cases.into_iter().enumerate() {
        assert_eq!(p1::longest_palindrome(input.to_string()), output, "case {} failed", i);
    }
}
...
fn test_convert() {
    ...
    for (i, (input, row, output)) in cases.into_iter().enumerate() {
        assert_eq!(p1::convert(input.to_string(), row), output, "case {} failed", i);
    }
}
...
fn test_three_sum() {
    ...
    for (i, (input, output)) in cases.into_iter().enumerate() {
        let h = p1::three_sum(input).into_iter().collect::<HashSet<Vec<i32>>>();
...
fn test_three_sum_closest() {
    ...
    for (i, (input, target, output)) in cases.into_iter().enumerate() {
        assert_eq!(p1::three_sum_closest(input, target), output, "case {} failed", i);
    }
}
...
fn test_letter_combinations() {
    ...
    for (i, (input, output)) in cases.into_iter().enumerate() {
        assert_eq!(p1::letter_combinations(input.to_string()).len(), output, "case {} failed", i);
    }
}
...
fn test_four_sum() {
    ...
    for (i, (input, target, output)) in cases.into_iter().enumerate() {
        let h = p1::four_sum(input, target).into_iter().collect::<HashSet<Vec<_>>>();
        assert_eq!(h.len(), output.len(), "case {} failed", i);
        for o in output {
...
fn test_remove_nth_from_end() {
    ...
    for (i, (input , target, output)) in cases.into_iter().enumerate() {
        assert_eq!(p1::remove_nth_from_end(ListNode::from_slice(input.as_slice()), target), ListNode::from_slice(output.as_slice()), "case {} failed", i);
    }
}
...
fn test_generate_parenthesis() {
    ...
    for (i, (input, output)) in cases.into_iter().enumerate() {
        let h = p1::generate_parenthesis(input).into_iter().collect::<HashSet<String>>();
        assert_eq!(h.len(), output.len(), "case {} failed", i);
        for o in output {
...
fn test_swap_pairs() {
    ...
    for (i, (input, output)) in cases.into_iter().enumerate() {
        assert_eq!(p1::swap_pairs(ListNode::from_slice(input.as_slice())), ListNode::from_slice(output.as_slice()), "case {} failed", i);
    }
}
