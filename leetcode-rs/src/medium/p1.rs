...
pub fn add_two_numbers(mut l1: Option<Box<ListNode>>, mut l2: Option<Box<ListNode>>) -> Option<Box<ListNode>> {
    ...
    loop {
        ...
        carry = if val < 10 { node.val = val; 0} else { node.val = val - 10; 1};
...
pub fn my_atoi(s: String) -> i32 {
    ...
    for &e in s.iter() {
        if e.is_ascii_digit() {
            match n.checked_mul(10).map(|x| x.checked_add(if sign > 0 {(e - b'0') as i32} else {-((e - b'0') as i32)})) {
                Some(Some(x)) => {
                    n = x;
                },
                _ => {
...
pub fn max_area(height: Vec<i32>) -> i32 {
    let (mut start, mut end, mut res) = (height.iter().enumerate().peekable(), height.iter().enumerate().rev().peekable(), 0);
...
pub fn int_to_roman(mut num: i32) -> String {
    ...
    const TABLES: [(i32, &str); 13] = [(1000, "M"), (900, "CM"), (500, "D"),
...
pub fn three_sum(mut nums: Vec<i32>) -> Vec<Vec<i32>> {
    nums.sort();
    let zero_idx = nums.iter().enumerate().rfind(|&(_, &x)| x == 0).map(|x| x.0).unwrap_or_default();
...
pub fn letter_combinations(digits: String) -> Vec<String> {
    ...
    let (codes, mut res) = match digits.is_empty() {
        true => {return vec![];}
        false => {
            let codes = digits.as_bytes().iter().map(|&x| TABLES[(x - b'2') as usize]).collect::<Vec<_>>();
            let elems = codes.iter().fold(1, |b, x| b * x.len());
            (codes, vec![String::new(); elems])
        }
    };

    codes.into_iter().for_each(|s| res.chunks_mut(s.len()).for_each(|r| {
        r.iter_mut().zip(s.chars()).for_each(|(r, c)| r.push(c))
    }));
...
pub fn four_sum(mut nums: Vec<i32>, target: i32) -> Vec<Vec<i32>> {
    nums.sort();
    let tgt_idx = nums.iter().enumerate().rfind(|&(_, &x)| x == target).map(|x| x.0).unwrap_or_else(|| nums.len().saturating_sub(1));
    let zero_idx = nums.iter().enumerate().find(|&(_, &x)| x == 0).map(|x| x.0).unwrap_or_else(|| nums.len().saturating_sub(1));
    let mut res = HashSet::new();
...
pub fn remove_nth_from_end(head: Option<Box<ListNode>>, n: i32) -> Option<Box<ListNode>> {
    ...
    let node = pre.next.as_mut().unwrap_or_else(|| unreachable!()).next.take();
    pre.next = node;
    ...
    for i in 1..=n {
        ...
        f.iter().take(i)
            ...
            .for_each(|(f_x, f_y)| {
                f_x.iter()
                    .for_each(|x| f_y.iter()
                        .for_each(|y| f_n.push(format!("({}){}", x, y))));
            });
...
pub fn swap_pairs(head: Option<Box<ListNode>>) -> Option<Box<ListNode>> {
    ...
    loop {
        ...
        if pre.next.as_ref().map(|cur| cur.next.is_some()) == Some(true) {
            ...
            next.next = Some(cur);
            pre.next = Some(next);
            pre = pre.next.as_mut().map(|cur| cur.next.as_mut().unwrap_or_else(|| unreachable!())).unwrap_or_else(|| unreachable!());
        } else {
            break;
        }
    }
...
pub fn divide(dividend: i32, divisor: i32) -> i32 {
    ...
    let (dividend, divisor) = (
        if dividend == i32::MIN { i32::MAX as u32 + 1} else {dividend.abs() as u32},
        if divisor == i32::MIN { i32::MAX as u32 + 1 } else {divisor.abs() as u32}
    );
    ...
    if dividend == 0 || divisor > dividend {
        ...
        let (mut sum, mut res, mut bit_len) = (0, 0, (u32::BITS - dividend.leading_zeros()) - (u32::BITS - divisor.leading_zeros()));
        ...
        loop {
            if bit_len == 0 {
                ...
                return if is_neg {
                    if res >= (i32::MAX as u32 + 1) {i32::MIN} else {-(res as i32)}
                } else {
                    if res >= (i32::MAX as u32) {i32::MAX} else {res as i32}
                };
            }
...
pub fn next_permutation(nums: &mut Vec<i32>) {
    ...
    let idx_a = nums.iter().enumerate().rev().skip(1)
        .zip(nums.iter().rev())
        .find(|((_, a), b)| a < b).map(|((i_a, &a), _)| {(i_a, a)});

    // find a[i] < a[j] in the range of [i+1, n) from tail to head
    if let Some((i_a, a)) = idx_a {
        if let Some(i_b) = nums.iter().enumerate().skip(i_a + 1).rev().find(|(_, &b)| a < b).map(|(i_b, _)| i_b) {
            nums.swap(i_a, i_b);
        }
...
pub fn search_range(nums: Vec<i32>, target: i32) -> Vec<i32> {
    let (mut left, mut right, mut idx) = (0 as i32, nums.len() as i32 - 1, None);
    ...
    if let Some(i) = idx {
        match (nums.iter().enumerate().take(i + 1).rev().find(|&(_, &e)| e != target).map(|e| e.0),
            nums.iter().enumerate().skip(i + 1).find(|&(_, &e)| e != target).map(|e| e.0)) {
            (Some(x), Some(y)) => {vec![x as i32 + 1, y as i32 - 1]},
            (Some(x), None) => {vec![x as i32 + 1, i as i32]},
            (None, Some(y)) => {vec![i as i32, y as i32 - 1]},
            (None, None) => {vec![i as i32, i as i32]},
        }
    } else {
        vec![-1, -1]
    }
