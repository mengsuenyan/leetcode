...
pub fn is_balanced(root: Option<Rc<RefCell<TreeNode>>>) -> bool {
    ...
    while let Some((node, level)) = stk.pop_front() {
        ...
        match node.borrow().left.clone() {
            ...
            Some(left) => stk.push_back((left, level + 1)),
        }
...
pub fn generate(num_rows: i32) -> Vec<Vec<i32>> {
    (1..num_rows).fold(vec![vec![1]], |mut pascal, nums| {
        ...
        for (&e1, &e2) in pascal
            ...
            .unwrap()
            .iter()
            .zip(pascal.last().unwrap().iter().skip(1))
        {
            row.push(e1 + e2);
        }
        row.push(1);
...
pub fn get_row(row_index: i32) -> Vec<i32> {
    (1..(row_index + 1))
        .fold((vec![1], vec![1]), |(pre, mut cur), _| {
            cur.clear();
            cur.push(1);
    ...
    prices
        ...
        .map(|&x| {
            prices
                .iter()
                .skip(1)
                .fold((0, x), |(profit, low_point), &val| {
                    (
                        std::cmp::max(profit, val - low_point),
                        std::cmp::min(low_point, val),
                    )
                })
                .0
        })
...
pub fn is_palindrome(s: String) -> bool {
    let (len, mut head, mut tail) = (
        s.len().saturating_sub(1),
        s.chars().enumerate().peekable(),
        s.chars().rev().enumerate().peekable(),
    );
...
impl MinStack {
    ...
    pub fn pop(&self) {
        if let Some(val) = unsafe { (&mut *self.data.as_ptr()).pop() } {
            if val < self.min.get() {
                let x = unsafe { &*self.data.as_ptr() }
                    .iter()
                    .min()
                    .copied()
                    .unwrap_or(i32::MAX);
